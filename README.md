...
- 阶段 1B：完成低 PDL 采样语义追溯；最终 low-PDL sampling profile 仍待研究者确认。
- 阶段 1B.1：完成 README 文档导航与交接可读性维护。

当前尚未完成：

- `PDL = 0.2 / 0.4 / 0.6 / 0.8` 的真实 tile 资产。
- Draco DRC。
...
3. `docs/PDL_SAMPLING_AUDIT.zh-CN.md`
4. `docs/DATA_PREP_CONTRACT.zh-CN.md`
5. `docs/DECISION_LOG.zh-CN.md`
...
| 文档 | 职责 | 适用场景 | 状态定位 |
| --- | --- | --- | --- |
| `docs/PDL_SAMPLING_AUDIT.zh-CN.md` | calibration 项目低 PDL 采样语义追溯与 tile-local 适配分析 | 冻结 low-PDL sampling profile 前必须阅读 | 阶段 1B 审查报告，D1B-1 仍待研究者确认 |
...
2. 未来若采用 tile-local low-PDL sampling，它属于 calibration sampling rule 的 derived adaptation，不应写成 tile-level calibrated quality evidence。
...
4. 当前已生成的真实点云资产只有 frame 1051、`PDL = 1.0` 的 binary PLY baseline。
