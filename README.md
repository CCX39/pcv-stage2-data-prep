...
当前仓库不是完整 Stage2 实验仓库；frame 1051 的 PLY source pilot 与 DRC delivery pilot 已完成，但尚未完成播放器 XML、Stage2Input、多帧或全序列批处理。
...
- 阶段 1D：完成 frame 1051、G128、40 个非空 tile 的五档 binary PLY 真实生成与完整验证。
- 阶段 1D.1：完成 multi-PDL artifact staging 发布健壮性与 root-level provenance 验证加固。
- 阶段 2A：完成 composite DRC variant 语义冻结、Draco CLI 无输入审查和 Stage2 理论接口交接文档。
- 阶段 2B / 2B.1：完成 frame 1051 双代表 tile 的 30-variant Draco round-trip probe，并在 order-independent validation contract 下通过独立验证。
- 阶段 2C：完成 frame 1051 全部 40 个非空 tile 的 600-variant DRC pilot corpus 生成，并通过独立 validator 核验。
...
- DRC-aware quality evidence 与 target-side decode benchmark。
...
5. `docs/PILOT_MULTIPDL_BINARY_ASSETS_CURRENT.zh-CN.md`
6. `docs/DRACO_TOOLCHAIN_AUDIT_CURRENT.zh-CN.md`
7. `docs/DRACO_ROUNDTRIP_PROBE_CURRENT.zh-CN.md`
8. `docs/PILOT_DRC_CORPUS_CURRENT.zh-CN.md`
9. `docs/STAGE2_COMPOSITE_VARIANT_THEORY_HANDOFF.zh-CN.md`
10. `docs/PDL_SAMPLING_AUDIT.zh-CN.md`
11. `docs/DATA_PREP_CONTRACT.zh-CN.md`
12. `docs/DECISION_LOG.zh-CN.md`
...
| `docs/PILOT_MULTIPDL_BINARY_ASSETS_CURRENT.zh-CN.md` | frame 1051 五档 binary PLY 资产的生成、metadata 与完整验证结果 | 检查 multi-PDL pilot 资产范围、数量、provenance 与验证结论 | 阶段 1D 资产记录 |
| `docs/DRACO_TOOLCHAIN_AUDIT_CURRENT.zh-CN.md` | 本机 Draco CLI 的只读事实审查、可观察 option 与未确认边界 | 进入 DRC round-trip probe 前阅读 | 阶段 2A 工具链审查 |
| `docs/DRACO_ROUNDTRIP_PROBE_CURRENT.zh-CN.md` | 两个代表 tile 的 30-variant Draco round-trip probe、order-independent 验证结果与边界 | 进入全量 600 DRC pilot corpus 前阅读 | 阶段 2B / 2B.1 probe 记录 |
| `docs/PILOT_DRC_CORPUS_CURRENT.zh-CN.md` | frame 1051 600-variant DRC pilot corpus 的规模、provenance 与验证边界 | 与 Stage2 frame 1051 pilot 对接前阅读 | 阶段 2C corpus 状态记录 |
...
| `docs/STAGE2_COMPOSITE_VARIANT_THEORY_HANDOFF.zh-CN.md` | PDL-only lookup 与 composite DRC variant 的理论接口交接 | 讨论 solver contract、lookup cap、variant-aware repair 或 Pareto pruning 前阅读 | 阶段 2A 理论交接 |
...
| `artifacts/` | 存放真实生成资产，例如 `artifacts/pilot_1051_g128_raw_v1/` 与 `artifacts/pilot_1051_g128_tilelocal_pdl5_v1/` | Git ignored，不进入版本库 |
...
4. 当前已生成的真实 pilot 资产限于 frame 1051：阶段 1A 的 `PDL = 1.0` binary PLY baseline、阶段 1D 的五档 binary PLY multi-PDL root，以及阶段 2C 的 600-variant DRC pilot corpus；尚未生成 XML、asset catalog、Stage2Input 或多帧资产。
5. 阶段 2C 后当前 active DRC variant dimension 为 `source_pdl × qp`，`cl = 10` 固定，`qc` 不进入当前 variant identity；DRC byte size 是 measured encoded file size，但不自动等价于最终网络开销、正式 `r_bytes` 或 `D(i,v)`。
6. 真实大体积资产不进入 Git；Git 仓库只保存代码、配置、中文文档和可审查的轻量元信息。
