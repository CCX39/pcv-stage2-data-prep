...
当前仓库不是完整 Stage2 实验仓库，也尚未完成全量 Draco DRC corpus、播放器 XML、Stage2Input 或全序列批处理。
...
- 全量 Draco DRC corpus（当前仅完成两个代表 tile 的 30-variant probe）。
...
8. `docs/STAGE2_COMPOSITE_VARIANT_THEORY_HANDOFF.zh-CN.md`
9. `docs/PDL_SAMPLING_AUDIT.zh-CN.md`
10. `docs/DATA_PREP_CONTRACT.zh-CN.md`
11. `docs/DECISION_LOG.zh-CN.md`
...
4. 当前已生成的真实点云资产仅限 frame 1051 pilot：阶段 1A 的 `PDL = 1.0` binary PLY baseline，以及阶段 1D 的五档 binary PLY multi-PDL root；尚未生成 DRC、XML、asset catalog、Stage2Input 或多帧资产。
5. 阶段 2B.1 后当前 active DRC variant dimension 为 `source_pdl × qp`，`cl = 10` 固定，`qc` 不进入当前 variant identity；当前 PLY source assets 不是最终完整 delivery candidate space。
