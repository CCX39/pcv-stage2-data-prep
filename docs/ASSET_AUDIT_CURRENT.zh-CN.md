...
以下信息由研究者根据既有项目经历提供，不是本次仅凭目录或文件名静态审查得到的结论：

1. 旧处理结果整体参考 DASH 风格组织。
2. `A1_ply` 与 `A2_drc` 属于早期 ASCII PLY 路径：
   - `A1_ply` 为 ASCII 格式的 tile PLY；
   - `A2_drc` 为与该早期路径相关的 Draco 压缩资产。
3. 后续实验发现 binary PLY 更适合作为点云中间资产，因此新数据准备项目不再需要兼容或复刻 ASCII PLY 路径。
4. 后续新管线优先关注：
   - binary PLY tile 资产；
   - 基于 binary PLY 生成的 DRC 候选传输资产；
   - 必要的、独立于旧播放器的可追溯 manifest。
5. `A1/A2` 可保留为历史参考，但不作为新 pipeline 的正式复用目标。
6. 导师脚本包中存在与当前数据准备目标无直接关系的历史代码、播放器相关代码、实验残留或辅助组件；后续只需关注其中与切块规则、binary PLY 转换、质量版本生成、Draco 编码和必要元数据生成直接相关的部分。
